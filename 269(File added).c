...
void saveStudentDataToFile(const char *filename)
{
    ...
    fwrite(studentDatabase, sizeof(struct Student), totalStudents, file);
    fclose(file);
    printf("Student data saved to file.\n");
}

// Function to load student data from a file
void loadStudentDataFromFile(const char *filename)
{
    FILE *file = fopen("student_data.txt", "rb");
    if (file == NULL)
    {
        perror("Error");
        return;
    ...
    fread(studentDatabase, sizeof(struct Student), totalStudents, file);
    fclose(file);
    printf("Student data loaded from file.\n");
}


// All FILE functions are here


// Function to save fund account data to a file
void saveFundDataToFile(const char *filename)
{
    FILE *file = fopen("fund_data.txt", "wb");
    if (file == NULL)
    {
        perror("Error");
        return;
    ...
    fwrite(fundDatabase, sizeof(struct FundAccount), totalAccounts, file);
    fclose(file);
    printf("Fund data saved to file.\n");
}

// Function to load fund account data from a file
void loadFundDataFromFile(const char *filename)
{
    FILE *file = fopen("fund_data.txt", "rb");
    if (file == NULL)
    {
        perror("Error");
        return;
    ...
    fread(fundDatabase, sizeof(struct FundAccount), totalAccounts, file);
    fclose(file);
    printf("Fund data loaded from file.\n");
}

int main()
{
    // Load previously saved data from files (if any)
    loadStudentDataFromFile("student_data.txt");
    loadFundDataFromFile("fund_data.txt");
    ...
    saveStudentDataToFile("student_data.txt");
    saveFundDataToFile("fund_data.txt");
